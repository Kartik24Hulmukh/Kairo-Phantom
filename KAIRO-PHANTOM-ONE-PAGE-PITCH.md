...
**Version:** 1.2 (operationally hardened) · **Date:** 11 July 2026 · *(No unverified traction. Users = 0, revenue = 0. Every metric is reproducible.)*
...
- **11 fixture-verified domain adapters**; **1008 tests passed, 4 skipped (all environmental; see SKIPS.md), 0 failed**; canary-breaks that turn CI red so "green" can't be faked.
...
The neutral-appraiser roadmap plans **read-only** adapters that normalize evidence from other agent/attestation ecosystems into the KSEE draft profile. Target sources include Microsoft AGT, Attested Intelligence AGA, Asqav / SCITT-style receipt chains, and OpenTelemetry / MCP action traces. Adapters imply **no endorsement** and are gated behind T4 (after market falsification). The full independent KSEE verifier is future work; today only supported signatures and hash chains are checked offline.
