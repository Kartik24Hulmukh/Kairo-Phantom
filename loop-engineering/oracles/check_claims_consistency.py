...
Checks:
  ...
  4. SKIPS.md has no unfilled <fill> rows AND the number of skip rows matches
     the skip count stated in CLAIMS.md R1 (derived, not hard-coded).
...
def main(argv):
    ...
    if skips is None:
        ...
        # Derive expected skip count from CLAIMS.md R1 (e.g. "1006 passed / 6 skipped")
        expected_skips = None
        if claims is not None:
            r1_match = re.search(r"R1\b.*?\|\s*\d+\s+passed\s*/\s*\**\s*(\d+)\s+skipped", claims)
            if r1_match:
                expected_skips = int(r1_match.group(1))
        if expected_skips is not None:
            if len(rows) != expected_skips:
                problems.append(f"SKIPS.md documents {len(rows)} skips; expected {expected_skips} to match CLAIMS.md R1")
        else:
            problems.append("Could not derive expected skip count from CLAIMS.md R1 \u2014 ensure R1 has '<n> passed / <n> skipped'")
