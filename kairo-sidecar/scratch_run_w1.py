...
def verify_prompt_builders():
    ...
    for name, builder_func in builders:
        ...
        assert between_reminder_and_instruction.strip() == "", (
            ...
            f"Found: {repr(between_reminder_and_instruction)}"
        )
...
if __name__ == "__main__":
    try:
        verify_prompt_builders()
        test_llm_caller_json_decode_retry()
        print("SCRATCH VERIFICATION SUCCESSFUL")
        sys.exit(0)
    except AssertionError as e:
        print(f"SCRATCH VERIFICATION FAILED: {e}", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        print(f"SCRATCH VERIFICATION UNEXPECTED ERROR: {e}", file=sys.stderr)
        sys.exit(1)
